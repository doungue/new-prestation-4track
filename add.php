...
<body>
	...
	if (isset($_POST['Submit'])) {
		...
		$id_annonce = mysqli_real_escape_string($mysqli, $_SESSION['type']);
		...
		if (empty($nama) || empty($umur) || empty($email) || empty($lieu) || empty($pays) || empty($site) || empty($service) || empty($domaine) || empty($details) || empty($filename)) {
			...
			if (empty($filename)) {
				echo "<font color='red'>veuillez entrer une image.</font><br/>";
			}
			...
			$result = mysqli_query($mysqli, "INSERT INTO users(nama,umur,email,lieu,pays,site,service, domaine,details,id_user,id_annonce,gambar) VALUES('$nama', '$umur', '$email', '$lieu', '$pays', '$site', '$service','$domaine', '$details','$id_user','$id_annonce', '$filename')");
